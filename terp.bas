...
MODE7:VDU23,0,10,8;0;0;0;23,0,11,12;0;0;0;:PRINT:DIM S% 255:L%=40:vpos%=17:*FX11,0
...
CLS:PRINT:PROCdRM2:NV%(0)=0:PROC360



REM *** MAIN GAME LOOP ***

REPEAT IF W%AND2 IF POS PRINT
PROCb:INPUT"Tell me what to do? "TP$:PRINT:PROC170
IF F% PRINT Y$:UNTIL0
...
NEXT:IFPOS PRINT':PROCv:ENDPROC ELSE PRINT:PROCv:ENDPROC
...
IF A%=0 ELSE IF A%<52 OR A%>101 PROCc:PROCp(MS$(A%+50*(A%>101))):IF W%AND1 ELSE IF W%AND2 ELSE IF POS PRINT
...
IF NOT L PROCcls
R%=K%:PROCdRM2:ENDPROC

REM 52. GETx
DEFPROC660:L=0:FORZ%=0TOIL%:L=L-IA%(Z%)=-1
NEXT
REMIF MX>=0 IF L>=MX PRINT Z$:Y%=4:ENDPROC ELSE PROC1050:IA%(P%)=-1:PROCdRM2:ENDPROC
IF MX>=0 IF L>=MX PRINT Z$:Y%=4:ENDPROC ELSE PROC1050:Z%=IA%(P%):IA%(P%)=-1:IF Z%=R% PROCdRM2:ENDPROC ELSE ENDPROC

REM 74. AGETx
DEFPROC692:PROC1050:Z%=IA%(P%):IA%(P%)=-1:IF Z%=R% PROCdRM2:ENDPROC ELSE ENDPROC

REM 53. DROPx
DEFPROC700:PROC1050:IA%(P%)=R%:PROCdRM2:ENDPROC
...
DEFPROC750:PROC1050:L=P%:PROC1050:Z%=IA%(P%):IA%(P%)=IA%(L):IA%(L)=Z%:IF IA%(P%)<>R% AND Z%<>R% ENDPROC ELSE IF IA%(P%)=Z% ENDPROC ELSE PROCdRM2:ENDPROC
...
DEFPROC840:PROC1050:L=P%:Z%=IA%(L):PROC1050:IA%(L)=P%:IF P%<>R% AND Z%<>R% ENDPROC ELSE IF P%=Z% ENDPROC ELSE PROCdRM2:ENDPROC

REM 75. BYx->x (Put Par #1 object in same room as Par #2 object)
DEFPROC842:PROC1050:L=P%:PROC1050:Z%=IA%(L):IA%(L)=IA%(P%):IFIA%(L)=R% ORZ%=R% IFZ%<>IA%(L) PROCdRM:ENDPROC EL.ENDPROC

REM 63. FINI
DEFPROC850:PROCn:PROCb:INPUT"THE GAME IS NOW OVER"'"ANOTHER GAME? "K$:IFLEFT$(K$,1)="N"END ELSE RUN

REM 64. DspRM
DEFPROCdRM:PROCdRM2:ENDPROC
...
REM 78. DSPCT
DEFPROC943:PROCc:PRINT;CT%;:ENDPROC
