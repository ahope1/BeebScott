...
REPEAT PROCb:INPUT"Tell me what to do? "TP$:PRINT:PROC170
IF F% PRINT Y$:UNTIL0
PROC360:IF LT<0 ELSE IFIL%<9 ELSE IFIA%(9)=0 ELSE IFCT%(8)=0 ELSE CT%(8)=CT%(8)-1:IFCT%(8)=0 SF%=SF% OR 2^16:PRINT"LIGHT HAS RUN OUT"ELSE IFCT%(8)<25PRINT"LIGHT RUNS OUT IN ";CT%(8);" TURN"CHR$(83*-(CT%(8)>1))"!"
...
REMIF A%=0 ELSE IF A%<52 OR A%>101 PROCp(MS$(A%+50*(A%>101))):IF NOT W% IF POS PRINT
IF A%=0 ELSE IF A%<52 OR A%>101 PROCp(MS$(A%+50*(A%>101))):IF W%AND1 ELSE IF POS PRINT
...
IFA%<64EL.IFA%>75EL.IFA%=64PROC860 EL.IFA%=65PROC870 EL.IFA%=66PROC890 EL.IFA%=67PROC920 EL.IFA%=68PROC930 EL.IFA%=69PROC940 EL.IFA%=70PROC950 EL.IFA%=71PROC710 EL.IFA%=72PROC750 EL.IFA%=73PROC955 EL.IFA%=74PROC692 EL.IFA%=75PROC842
IFA%<76EL.IFA%>87EL.IFA%=76PROC860 EL.IFA%=77PROC942 EL.IFA%=78PROC943 EL.IFA%=79PROC944 EL.IFA%=80PROC742 EL.IFA%=81PROC945 EL.IFA%=82PROC946 EL.IFA%=83PROC947 EL.IFA%=84PROC951 EL.IFA%=85PROC952 EL.IFA%=86PROC953 EL.IFA%=87PROC743
IFA%=88PROC745
...
REMIF F PRINT"I DON'T UNDERSTAND YOUR COMMAND":ENDPROC
...
REMIF NV%(0)<>10 ELSE L=0:FORZ%=0TOIL%:L=L-IA%(Z%)=-1:NEXT:IF MX>=0 IF L>=MX PRINTZ$:F%=0:ENDPROC
...
REM IFNV%(0)=10 ELSE IFIA%(X%)<>-1 K%=1:NEXT:PROC1200:ENDPROC ELSE IA%(X%)=R%:K%=3
...
NEXT
IF MX>=0 IF L>=MX PRINT Z$:Y%=4:ENDPROC ELSE PROC1050:IA%(P%)=-1:ENDPROC
...
DEFPROCo($S%):LOCALA%,Z%,C%,N%:N%=LEN$S%:A%=0:Z%=L%+1-POS:REPEATIFZ%>N%Z%=N%ELSEREPEATZ%=Z%-1:C%=S%?Z%:UNTILC%=32:IF Z%>=A% ELSE IF POS PRINT:Z%=L%+1:UNTIL0 ELSE STOP
S%?Z%=&D:PRINT$(S%+A%);:IFZ%<N% IF POS PRINT
A%=Z%+1:Z%=A%+L%+1:UNTILA%>=N%:ENDPROC

REM Backtrack to previous non-space char 
REMDEFPROCj:LOCALA%,C%:A%=&87:REPEATVDU8:C%=(USR(&FFF4)AND&FF00)DIV256:UNTILC%<>32:VDU9:ENDPROC
...
REMDEFPROCb:IFPOS ENDPROC ELSE IF VPOS=0 IF POS=0 ENDPROC ELSE IF FNb ENDPROC ELSE PRINT:ENDPROC
