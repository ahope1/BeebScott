...
[***example.sh***](https://github.com/ahope1/BeebScott/blob/main/code/example.sh) **is the list of commands used in the how-to video linked below.** The commands convert the DAT game file for Adventureland (Scott Adams's first text adventure game) for use with BeebScott and add the converted file to *template.ssd*, together with the other files required. 
