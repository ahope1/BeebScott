...
MODE7:VDU23,0,10,8;0;0;0;23,0,11,12;0;0;0;10:DIM S% 255:B%=1:*FX11,0
...
IF LENNT$(0)=1 IF LENNT$(1)=0 Z%=INSTR("NSEWUD",NT$(0)):IF Z% NV%(0)=1:NV%(1)=Z%:F%=0:K$=TP$:ENDPROC
...
NEXT:K$=TP$:F%=NV%(0)<1
REM Handle just NORTH or SOUTH etc.
IF NOT F% ELSE FORX%=1TO6:IF LEFT$(NV$(X%,1),ln%)=LEFT$(NT$(0),ln%) NV%(0)=1:NV%(1)=X%:F%=0:NEXT ELSE NEXT
REM If verb unrecognised then return. If noun absent then return. If noun present and unrecognised then flag and return.
IF F% ENDPROC ELSE IFLEN(NT$(1))>0 IF NV%(1)<1 F4%=-1:F%=0:ENDPROC ELSE F4%=0:ENDPROC
...
IF SF% AND &8000 IF IA%(9)<>-1 AND IA%(9)<>R% PRINT"I CAN'T SEE, ITS TOO DARK.":PROCv:B%=0:ENDPROC
K%=-1:IFLEFT$(RS$(R%),1)="*"PROCo(MID$(RS$(R%),2))ELSEPROCo("I'm in a "+RS$(R%))
PROCn
FORZ%=0TOIL%:IF K% IF IA%(Z%)=R% PRINT SPC40 "VISIBLE ITEMS HERE:" SPC101;:V.11,11:K%=0
...
NEXT:PROCn:PRINT SPC40;:PROCv:B%=0:ENDPROC
...
IFA%<64EL.IFA%>75EL.IFA%=64PROCdRM EL.IFA%=65PROC870 EL.IFA%=66PROC890 EL.IFA%=67PROC920 EL.IFA%=68PROC930 EL.IFA%=69PROC940 EL.IFA%=71PROC710 EL.IFA%=72PROC750 EL.IFA%=73cont%=-1 EL.IFA%=74PROC692 EL.IFA%=75PROC842
...
REM 10=GET, 18=DROP
DEFPROC1060
IF NV%(1)=0 PRINT"WHAT?":F%=0:ENDPROC ELSE IF NV%(0)<>10 ELSE IF MX%<0 ELSE L%=0:FORZ%=0TOIL%:L%=L%-(IA%(Z%)=-1):NEXT:IF L%>=MX% PROCo(Z$):PROCn:F%=0:ENDPROC
K%=0:FOR X%=0TOIL%:$S%=IA$(X%):IF S%?(LEN$S%-1)<>47 NEXT:PROC1200:ENDPROC ELSE S%?(LEN$S%-1)=&D:Y%=INSTR($S%,"/"):IF Y%=0 STOP
...
REM 55/59. x->RM0
DEFPROC760:PROC1050:Z%=IA%(P%):IA%(P%)=0:IF Z%=R% B%=1:ENDPROC ELSE ENDPROC
...
DEFPROC940:SF%=SF% AND NOT &10000:CT%(8)=LT%:IA%(9)=-1:B%=1:ENDPROC
