...
MODE7:VDU23,0,10,8;0;0;0;23,0,11,12;0;0;0;10:DIM S% 255:B%=TRUE:*FX11,0

DIM arr% 5,CT%(8):CT%=0:!arr%=0:arr%!2=0:K%=0:TP$=STRING$(40,"*"):TP$="":Z$="I'VE TOO MUCH TOO CARRY. TRY -TAKE INVENTORY-":Y$="YOU USE WORD(S) I DON'T KNOW":PROC1240:*FX21,0

R%=AR%:CT%(8)=LT%:SF%=0:INPUT"USE OLD 'SAVED' GAME? "K$
IF LEFT$(K$,1)="Y" INPUT"READY SAVED TAPE "K$:d=OPENIN("SAV"):INPUT#d,SF%,R%,CT%:FOR X%=0 TO IL%:INPUT#d,IA%(X%):NEXT:FOR X%=0TO8:INPUT#d,CT%(X%):NEXT:FOR X%=0TO5:arr%?X%=BGET#d:NEXT:CLOSE#d
...
REPEAT IF B% PROCdRM
IF POS PRINT
PROCb:VDU23,1,1;0;0;0;:INPUT"Tell me what to do? "TP$:VDU23,1,0;0;0;0;:PRINT:PROC170
IF F% PRINT Y$:UNTIL0
PROC360:IF LT%<0 EL. IFIL%<9 EL. IFIA%(9)=0 ELSE IFCT%(8)=0 ELSE CT%(8)=CT%(8)-1:PROCn:IFCT%(8)=0 SF%=SF% OR &10000:PRINT"LIGHT HAS RUN OUT"ELSE IFCT%(8)<25PRINT"LIGHT RUNS OUT IN ";CT%(8);" TURN"CHR$(83*-(CT%(8)>1))"!"
...
IF LENNT$(0)=1 IF LENNT$(1)=0 Z%=INSTR("NSEWUD",NT$(0)):IF Z% NV%(0)=1:NV%(1)=Z%:F%=FALSE:K$=TP$:ENDPROC
FORX%=0TO1:Z%=0:NV%(X%)=0:IFNT$(X%)=""Z%=1 ELSE FORY%=0TONL%:K$=NV$(Y%,X%):IFLEFT$(K$,1)="*"K$=MID$(K$,2)
REM Truncate "directional nouns"
...
NEXT:K$=TP$:F%=NV%(0)<1
REM Just NORTH or just SOUTH etc.
IF NOT F% ELSE FORX%=1TO6:IF LEFT$(NV$(X%,1),ln%)=LEFT$(NT$(0),ln%) NV%(0)=1:NV%(1)=X%:F%=FALSE:NEXT ELSE NEXT
REM If verb unrecognised then return. If noun absent then return. If noun present and unrecognised then flag and return.
IF F% ENDPROC ELSE IFLEN(NT$(1))>0 IF NV%(1)<1 F4%=TRUE:F%=FALSE:ENDPROC ELSE F4%=FALSE:ENDPROC
...
IF SF% AND &8000 IF IA%(9)<>-1 IF IA%(9)<>R% PRINT"I CAN'T SEE, ITS TOO DARK.":PROCv:B%=FALSE:ENDPROC
K%=TRUE:IFLEFT$(RS$(R%),1)="*"PROCo(MID$(RS$(R%),2))ELSEPROCo("I'm in a "+RS$(R%))
PROCn
FORZ%=0TOIL%:IF K% IF IA%(Z%)=R% PRINT SPC40 "VISIBLE ITEMS HERE:" SPC101;:V.11,11:K%=FALSE
IF IA%(Z%)=R% PROC280:PROCo(CHR$(32*-(POS>0))+TP$+".")
NEXT:IF POS=0 PRINT SPC40;CHR$11; ELSE IF K% ELSE PROCn
K%=TRUE:FORZ%=0TO5:IF K% IFRM%(R%,Z%)<>0PRINT SPC120:V.11,11,11:P."OBVIOUS EXITS: ";:K%=FALSE
IFRM%(R%,Z%)<>0PRINTNV$(Z%+1,1);" ";
NEXT:PROCn:PRINT SPC40;:PROCv:B%=FALSE:ENDPROC
...
DEFPROC280:TP$=IA$(Z%):IFRIGHT$(TP$,1)="/" TP$=LEFT$(TP$,INSTR(TP$,"/")-1):ENDPROC ELSE ENDPROC


REM *** Search for matching actions ***

REM set up the success/failure flags
DEFPROC360:F2%=TRUE:F%=TRUE:F3%=FALSE

REM clear the continue-flag
cont%=FALSE

REM GO {N/S/E/W/U/D}
IFNV%(0)=1 IF NV%(1)<7 PROC610:ENDPROC
...
IF NV%(0)=0 OR cont% IFV%<>0 X%=CL%:NEXT:cont%=FALSE:ENDPROC

REM If continue-flag is set: 
REM  If V is zero (see above) but N isn't, then reset the continue-flag:
REM   If auto action, then process conditions, else error. 
REM IF NOT cont% ELSE IF V%<>0 STOP ELSE IF N%=0 ELSE cont%=FALSE:IF NV%(0)=0 ELSE STOP
IF NOT cont% ELSE IF N%=0 ELSE cont%=FALSE:IF NV%(0)=0 ELSE STOP
...
IF cont% ELSE IF NV%(0)<>0 ELSE F%=FALSE:IF RND(100)>N% NEXT:PROC990:ENDPROC
...
F2%=TRUE:F%=FALSE:F3%=TRUE:FORY%=1TO5:WW%=CA%(X%,Y%):LL%=WW% DIV20:K%=WW% MOD20:F1%=TRUE
...
IF A%=0 ELSE IF A%>51 IF A%<102 ELSE PROCc:PROCp(MS$(A%+50*(A%>101))):IF W%AND1 ELSE IF W%AND2 ELSE IF POS PRINT
IFA%<52EL.IFA%>63EL.IFA%=52PROC660 EL.IFA%=53PROC700 EL.IFA%=54PROC740 EL.IFA%=55PROC760 EL.IFA%=56PROC770 EL.IFA%=57PROC780 EL.IFA%=58PROC790 EL.IFA%=59PROC760 EL.IFA%=60PROC810 EL.IFA%=61PROC830 EL.IFA%=62PROC840 EL.IFA%=63PROC850
IFA%<64EL.IFA%>75EL.IFA%=64PROCdRM EL.IFA%=65PROC870 EL.IFA%=66PROC890 EL.IFA%=67PROC920 EL.IFA%=68PROC930 EL.IFA%=69PROC940 EL.IFA%=71PROC710 EL.IFA%=72PROC750 EL.IFA%=73cont%=TRUE EL.IFA%=74PROC692 EL.IFA%=75PROC842
IFA%<76EL.IFA%>87EL.IFA%=76PROCdRM EL.IFA%=77PROC942 EL.IFA%=78PROC943 EL.IFA%=79PROC944 EL.IFA%=80PROC742 EL.IFA%=81PROC945 EL.IFA%=82PROC946 EL.IFA%=83PROC947 EL.IFA%=84PROC951 EL.IFA%=85PROC952 EL.IFA%=86PRINT EL.IFA%=87PROC743
IFA%=88PROC745
...
DEFPROC990
IFNV%(0)=0 OR cont% cont%=FALSE:ENDPROC
REM        10=GET,        18=DROP
...
REM 10=GET, 18=DROP
DEFPROC1060
IF NV%(1)=0 PRINT"WHAT?":F%=FALSE:ENDPROC ELSE IF NV%(0)<>10 ELSE IF MX%<0 ELSE L%=0:FORZ%=0TOIL%:L%=L%-(IA%(Z%)=-1):NEXT:IF L%>=MX% PROCo(Z$):PROCn:F%=FALSE:ENDPROC
K%=FALSE:FOR X%=0TOIL%:$S%=IA$(X%):IF S%?(LEN$S%-1)<>47 NEXT:PROC1200:ENDPROC ELSE S%?(LEN$S%-1)=&D:Y%=INSTR($S%,"/"):IF Y%=0 STOP
IF$(S%+Y%)<>NV$(NV%(1),1)NEXT:PROC1200:ENDPROC ELSE IFNV%(0)=10 ELSE IFIA%(X%)<>-1 K%=1:NEXT:PROC1200:ENDPROC ELSE IA%(X%)=R%:K%=3
IFNV%(0)<>10 ELSE IFIA%(X%)<>R% K%=2:NEXT:PROC1200:ENDPROC ELSE IA%(X%)=-1:K%=3
PRINT"OK. ";:B%=TRUE:X%=IL%:NEXT
F%=FALSE:ENDPROC

DEFPROC1200:IF K%=1 PRINT"I'M NOT CARRYING IT" ELSE IFK%=2 PRINT"I DON'T SEE IT HERE"
IF K%=FALSE IF NOT F3% PRINT"ITS BEYOND MY POWER TO DO THAT":F%=FALSE
IF K%<>0 F%=FALSE
ENDPROC
...
DEFPROC1240:INPUT"READY DATA TAPE. HIT ENTER "d
d=OPENIN("DAT")
REM flags,objects,actions,words,rooms,mx,start,treasures,wordlen,light,messages,treasury
INPUT#d,W%,IL%,CL%,NL%,RL%,MX%,AR%,TT%,ln%,LT%,ML%,TR%
...
DEFPROC410:F1%=TRUE:FORZ%=0TOIL%:IFIA%(Z%)=-1 Z%=IL%:NEXT:ENDPROC ELSENEXT:F1%=FALSE:ENDPROC

REM 11. -ANY. Is player carrying nothing?
DEFPROC420:F1%=FALSE:FORZ%=0TOIL%:IFIA%(Z%)=-1 Z%=IL%:NEXT:ENDPROC ELSENEXT:F1%=TRUE:ENDPROC
...
REM Movement
DEFPROC610:L%=(SF% AND &8000)>0:IFL% L%=L% AND IA%(9)<>R% AND IA%(9)<>-1:IF L% PRINT"DANGEROUS TO MOVE IN THE DARK!"
IFNV%(1)<1PRINT"GIVE ME A DIRECTION TOO.":ENDPROC
K%=RM%(R%,NV%(1)-1):IFK%>=1 ELSE IFL% PRINT"I FELL DOWN AND BROKE MY NECK.":K%=RL%:SF%=SF% AND NOT &8000 ELSE PRINT"I CAN'T GO IN THAT DIRECTION":ENDPROC
R%=K%:B%=TRUE:ENDPROC

REM 52. GETx
DEFPROC660:L%=0:FORZ%=0TOIL%:L%=L%-(IA%(Z%)=-1)
NEXT
IF MX%>=0 IF L%>=MX% PROCo(Z$):PROCn:Y%=4:ENDPROC ELSE PROC1050:Z%=IA%(P%):IA%(P%)=-1:IF Z%=R% B%=TRUE:ENDPROC ELSE ENDPROC

REM 74. AGETx
DEFPROC692:PROC1050:Z%=IA%(P%):IA%(P%)=-1:IF Z%=R% B%=TRUE:ENDPROC ELSE ENDPROC

REM 53. DROPx
DEFPROC700:PROC1050:IA%(P%)=R%:B%=TRUE:ENDPROC

REM 71. SAVE
DEFPROC710:PRINT"SAVING GAME":INPUT"READY OUTPUT TAPE "d
...
REM 54. GOTOy
DEFPROC740:PROC1050:R%=P%:B%=TRUE:ENDPROC

REM 80. EXRM0
DEFPROC742:Z%=R%:R%=?arr%:?arr%=Z%:B%=TRUE:ENDPROC

REM 87. EXC,CR
DEFPROC743:PROC1050:Z%=R%:R%=arr%?P%:arr%?P%=Z%:B%=TRUE:ENDPROC
...
DEFPROC750:PROC1050:L%=P%:PROC1050:Z%=IA%(P%):IA%(P%)=IA%(L%):IA%(L%)=Z%:IF IA%(P%)<>R% IF Z%<>R% ENDPROC ELSE IF IA%(P%)=Z% ENDPROC ELSE B%=TRUE:ENDPROC

REM 55/59. x->RM0
DEFPROC760:PROC1050:Z%=IA%(P%):IA%(P%)=0:IF Z%=R% B%=TRUE:ENDPROC ELSE ENDPROC

REM 56. NIGHT
DEFPROC770:SF%=SF% OR &8000:ENDPROC

REM 57. DAY
DEFPROC780:SF%=SF% AND NOT &8000:ENDPROC
...
REM 61. DEAD
DEFPROC830:PRINT"I'M DEAD...":R%=RL%:SF%=SF% AND NOT &8000:PROCdRM:ENDPROC

REM 62. x->y (Move the Par #1 object to the Par #2 room)
DEFPROC840:PROC1050:L%=P%:Z%=IA%(L%):PROC1050:IA%(L%)=P%:IF P%<>R% IF Z%<>R% ENDPROC ELSE IF P%=Z% ENDPROC ELSE B%=TRUE:ENDPROC

REM 75. BYx->x (Put Par #1 object in same room as Par #2 object)
DEFPROC842:PROC1050:L%=P%:PROC1050:Z%=IA%(L%):IA%(L%)=IA%(P%):IFIA%(L%)=R% ORZ%=R% IFZ%<>IA%(L%) B%=TRUE:ENDPROC EL.ENDPROC

REM 63. FINI
DEFPROC850:IFB% PROCdRM
PROCn:PROCb:INPUT"THE GAME IS NOW OVER"'"ANOTHER GAME? "K$:IFLEFT$(K$,1)="N"END ELSE RUN

REM 64/76. DspRM
REMDEFPROCdRM:PROCdRM2:B%=0:ENDPROC

REM 65. SCORE
DEFPROC870:L%=0:FORZ%=0TOIL%:IFIA%(Z%)=TR% IFLEFT$(IA$(Z%),1)="*" L%=L%+1
NEXT:PRINT"I'VE STORED ";L%;" TREASURES."'"ON A SCALE OF 0 TO 100 THAT RATES A ";100*L% DIV TT%:IFL%=TT% PRINT"WELL DONE.":PROC850 EL.ENDPROC

REM 66. INV
DEFPROC890:PROCc:PRINT"I'M CARRYING:":WW%=TRUE:FORZ%=0TOIL%:IFIA%(Z%)=-1 WW%=FALSE:PROC280:PROCo(TP$+". ")
...
DEFPROC940:SF%=SF% AND NOT &10000:CT%(8)=LT%:IA%(9)=-1:B%=TRUE:ENDPROC

REM 77. CT-1
DEFPROC942:CT%=CT%-1:ENDPROC

REM 78. DSPCT
DEFPROC943:PROCc:PRINT;CT%;:IF (W%AND2)=FALSE PRINT" ";:ENDPROC ELSE ENDPROC
...
REM 70. CLS
REMDEFPROCcls:ENDPROC

REM 84. SAYW
DEFPROC951:PRINT MID$(K$,1+INSTR(K$," "));:ENDPROC

REM 85. SAYWCR
DEFPROC952:PROC951:PRINT:ENDPROC

REM 86. SAYCR
REMDEFPROC953:PRINT:ENDPROC

REM 73. CONT
REMDEFPROC955:cont%=-1:ENDPROC
...
DEFFNb:LOCALA%,C%,I%:VDU11:IF HIMEM<&7C01 ELSE VDU8:A%=&87:FORI%=1TO40:VDU9:C%=(USR(&FFF4)AND&FF00)DIV256:IFC%=32NEXT:VDU10,13:=TRUE ELSEI%=40:NEXT:VDU10,13:=FALSE
A%=?&34B*256+?&34A:VDU 10:FOR I%=0 TO 39 STEP 4
C%=A%+I%:C%=C%+&400*(C%>&7FFF)
IF !C%=&20202020 NEXT:=TRUE ELSE I%=39:NEXT:=FALSE
