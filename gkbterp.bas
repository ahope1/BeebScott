...
REM365 FORX=-I%*(NV(0)<>0)TOCL:V=INT(CA(X,0)/150):N=CA(X,0)-V*150
365 FORX=-I%*(NV(0)<>0)TOCL:V=FNca(X,0)DIV150:N=FNca(X,0)-V*150
...
400 F2=-1:F=0:F3=-1:FORY=1TO5:W=FNca(X,Y):LL=W DIV20:K=W-LL*20:F1=-1:ON K+1 GOTO550,430,450,470,490,500,510,520,530,540,410,420,440,460,480,542,543,546,547,544
...
530 F1=SF AND 2^LL:F1=F1<>0:GOTO550

REM 9. -BIT. Is numbered flag-bit clear?
540 F1=SF AND 2^LL:F1=F1=0:GOTO550
...
560 IP=0:FORY=1TO4:K=(Y-1)DIV2+6:ON Y GOTO570,580,570,580
570 AC=FNca(X,K)DIV150:GOTO590
580 AC=FNca(X,K)-(FNca(X,K)DIV150)*150
...
REM 52. GETx
660 L=0:FORZ=0TOIL:IFIA(Z)=-1 L=L+1
670 NEXTZ
...
REM 71. SAVE
710 PRINT"SAVING GAME":IFD=-1 INPUT"READY OUTPUT TAPE "K$:PRINT;(IL*5DIV60)+1;" MINUTES"ELSE OPEN"O",D,SV$
...
REM 58. SETz
790 GOSUB1050
800 SF=SF OR 2^P:GOTO960

REM 60. CLRz
810 GOSUB1050
820 SF=SF AND NOT (2^P):GOTO960
...
880 NEXTZ:PRINT"I'VE STORED ";L;" TREASURES."'"ON A SCALE OF 0 TO 100 THAT RATES A ";L DIV TT*100:IFL=TT THENPRINT"WELL DONE.":GOTO850 ELSE960
...
1050 IP=IP+1:W=FNca(X,IP):P=W DIV20:M=W-P*20:IF M<>0 THEN1050 ELSE RETURN
...
REM1270 DIMNV(1),CA(CL,7),NV$(NL,1),IA$(IL),IA(IL),RS$(RL),RM(RL,5),MS$(ML),NT$(1),I2(IL)
1270 DIMNV(1),CA% 17*CL-1,NV$(NL,1),IA$(IL),IA(IL),RS$(RL),RM(RL,5),MS$(ML),NT$(1),I2(IL)
REM1280 FORX=0TOCL:INPUT#d,CA(X,0),CA(X,1),CA(X,2),CA(X,3),CA(X,4),CA(X,5),CA(X,6),CA(X,7):NEXT
1280 Z=0:FORX=0TOCL:FORY=0TO15:CA%?Z=BGET#d:Z=Z+1:NEXTY,X
1282 INPUT#d,I%
...
2140 DEFFNca(X%,Y%)=(CA%?(X%*16+Y%*2))*256+CA%?(X%*16+Y%*2+1)
